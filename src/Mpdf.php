...
class Mpdf implements \Psr\Log\LoggerAwareInterface
{
	...
	function Image($file, $x, $y, $w = 0, $h = 0, $type = '', $link = '', $paint = true, $constrain = true, $watermark = false, $shownoimg = true, $allowvector = true)
	{
		...
		/* -- WATERMARK -- */
		if ($watermark && $watermark instanceof WatermarkImage) {
            $watermark_size = $watermark->getSize();
            $watermark_pos = $watermark->getPosition();
            $watermarkImageAlpha = $watermark->getAlpha();
            $watermarkImgAlpha = $this->SetAlpha($watermarkImageAlpha, 'Normal', true);
            $watermarkImgBehind=$watermark->getAlphaBlend();
			$maxw = $this->w;
			$maxh =  $this->h;
			// Size = D PF or array
			if (is_array($watermark_size)) {
				$w = $watermark_size[0];
				$h = $watermark_size[1];
			} elseif (!is_string($watermark_size)) {
				$maxw -= $watermark_size * 2;
				$maxh -= $watermark_size * 2;
				$w = $maxw;
				$h = abs($w * $info['h'] / $info['w']);
				if ($h > $maxh) {
					$h = $maxh;
			...
			if ($watermarkImgBehind) {
				$outstring = $watermarkImgAlpha . "\n" . $outstring . "\n" . $this->SetAlpha(1, 'Normal', true) . "\n";
				$this->pages[$this->page] = preg_replace('/(___BACKGROUND___PATTERNS' . $this->uniqstr . ')/', "\n" . $outstring . "\n" . '\\1', $this->pages[$this->page]);
			} else {
				$this->writer->write($outstring);
			}
	...
	// add a watermark
	function watermark(WatermarkText $watermark)
	{
		if ($this->PDFA || $this->PDFX) {
			throw new \Mpdf\MpdfException('PDFA and PDFX do not permit transparency, so mPDF does not allow Watermarks!');
		}
        $watermark_font = $this->default_font;
		if ($watermark->getFont()) {
            $watermark_font = $watermark->getFont();
		}
        $texte=$watermark->getText();
        $fontsize=$watermark->getSize();
        $alpha=$watermark->getAlpha();
        $angle=$watermark->getAngle();
		$this->SetFont($watermark_font, "B", $fontsize, false); // Don't output
		...
		$color = $watermark->getColor() ? $watermark->getColor() : 0;
        ...
        $w=$watermark->getW();
        $h=$watermark->getH();
		$maxlen = (min($w, $h) ); // sets max length of text as 7/8 width/height of page

		while ($loop == 0) {
			$this->SetFont($watermark_font, "B", $szfont, false); // Don't output
		...
		$this->SetFont($watermark_font, "B", $szfont - 0.1, true, true); // Output The -0.1 is because SetFont above is not written to PDF
	...
	function watermarkImg(WatermarkImage $watermark)
	{
		if ($this->PDFA || $this->PDFX) {
			throw new \Mpdf\MpdfException('PDFA and PDFX do not permit transparency, so mPDF does not allow Watermarks!');
		}
        $watermarkImage = $watermark->getPath();
        $watermarkImgBehind = $watermark->isBehindContent();
        $watermarkImgAlphaBlend = $watermark->getAlphaBlend();
		if (!$watermarkImgBehind) {
			$this->SetAlpha($watermark->getAlpha(), $watermarkImgAlphaBlend);
		}

		$this->Image($watermarkImage, 0, 0, 0,0, '', '', true, true, $watermark);
	...
	function Footer()
	{
		...
		foreach ($this->watermarks as $watermark){
            if ($watermark instanceof WatermarkText){
                $this->watermark($watermark);
            }elseif ($watermark instanceof WatermarkImage){
                $this->watermarkImg($watermark);
            }
